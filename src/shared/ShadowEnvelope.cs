...
{
    [JsonPropertyName("state")] public DesiredWrapper<T> State { get; init; } = new();
...
{
    [JsonPropertyName("desired")] public T? Desired { get; init; }
}

/// <summary>Wraps a reported state update: { "state": { "reported": T } }</summary>
public class ShadowReportedUpdate<T>
{
    [JsonPropertyName("state")] public ReportedWrapper<T> State { get; init; } = new();
...
{
    [JsonPropertyName("reported")] public T? Reported { get; init; }
}

/// <summary>Deserialises an incoming shadow delta: { "state": T }</summary>
public class ShadowDeltaMessage<T>
{
    [JsonPropertyName("state")] public T? State { get; init; }
}
